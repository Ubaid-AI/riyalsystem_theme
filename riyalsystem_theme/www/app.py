...
def get_context(context):
    ...
    theme_color_raw = theme_settings_list.get('theme_color') or 'Blue'
    theme_color_name = theme_color_raw.replace(' ', '-').lower()
    theme_color_class = theme_color_raw.replace(' ', '-')
    effective_menu_opening_type = (
        theme_settings_list.get('default_type')
        or theme_settings_list.get('menu_opening_type')
        or 'Workspace'
    )
    theme = 'light'

    if theme_settings_list.get('apply_dark_mode') == '1':
        theme = 'dark'
