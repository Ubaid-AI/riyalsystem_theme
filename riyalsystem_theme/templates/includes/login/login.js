...
//
// Riyal System Theme — login page behaviour (based on Data Value Theme)
//
...
login.reset_sections = function (hide) {
	if (hide || hide === undefined) {
		...
		$("section.for-twofactor").toggle(false);
	}
...
login.twofactor = function () {
	login.reset_sections();
	$(".for-twofactor").toggle(true);
	$("#login_token").focus();
};
...
login.login_handlers = (function () {
	...
	var login_handlers = {
		200: function (data) {
			...
			// OTP verification — only when the server explicitly returns verification data
...
var verify_token = function () {
	$(".form-verify").off("submit.rst-login").on("submit.rst-login", function (eventx) {
...
var request_otp = function () {
	// Show the dedicated 2FA section — never replace or empty the login form DOM.
	$("#otp_div").empty();
	$("#login_token").val("");
	verify_token();
	login.twofactor();
...
// Rotate login-card images when a slideshow is configured in Theme Settings.
(function () {
	var imgs = window.login_card_images || [];
	if (imgs.length < 2) return;
	var groups = document.querySelectorAll('.login-card-media');
	if (!groups.length) return;
	var idx = 0;
	setInterval(function () {
		var next = (idx + 1) % imgs.length;
		groups.forEach(function (g) {
			var slides = g.querySelectorAll('.login-card-slide');
